...
class SyntheticDataPipeline {
  ...
  _filterClausesByClassification(classifiedClauses) {
    if (this.classFilter === "all" && !this.prioritizeImportant) {
      return classifiedClauses;
    }

    // If we need to filter by class
    if (this.classFilter !== "all") {
      const allowedClassifications = this.classFilter.split("_");
      classifiedClauses = classifiedClauses.filter((clause) =>
        allowedClassifications.includes(clause.classification.toLowerCase())
      );
    }

    // If we need to prioritize important and critical content
    if (this.prioritizeImportant) {
      // Sort clauses by priority (Critical > Important > Standard)
      classifiedClauses.sort((a, b) => {
        const priorityMap = {
          "Critical": 3,
          "Important": 2,
          "Standard": 1
        ...
        return priorityMap[b.classification] - priorityMap[a.classification];
      });

      console.log(`Prioritizing important content. Total clauses: ${classifiedClauses.length}`);
      
      // Calculate token estimation (rough estimate)
      const totalEstimatedTokens = classifiedClauses.reduce((sum, clause) => 
        sum + Math.ceil(clause.input.length / 4), 0);
      
      console.log(`Estimated total tokens: ${totalEstimatedTokens}`);
      
      // If we're over a threshold (2000 tokens), limit to critical and important only
      if (totalEstimatedTokens > 2000) {
        const limitedClauses = classifiedClauses.filter(clause => 
          clause.classification === "Critical" || clause.classification === "Important");
        
        // If we still have enough clauses after filtering
        if (limitedClauses.length > 0) {
          console.log(`Limiting to ${limitedClauses.length} Critical/Important clauses due to token constraints`);
          ...
          .map((v) =>
            JSON.stringify({
              messages: [
                {
                  role: "system",
                  content: "You are a legal document expert that rewrites legal clauses to maintain legal meaning while changing the specific wording and phrasing.",
                },
          ...
          .map((v) =>
            JSON.stringify({
              messages: [
                {
                  role: "system",
                  content: "You are a Standard Operating Procedure (SOP) expert that rewrites technical instructions to maintain operational accuracy while using different phrasing.",
                },
                { role: "user", content: v.input },
                { role: "assistant", content: v.output },
              ],
          ...
          .map((v) =>
            JSON.stringify({
              messages: [
                {
                  role: "system",
                  content: "You are an expert at converting informational statements into question-answer pairs that retain the original information.",
                },
                { role: "user", content: `Convert this statement into a question-answer pair: ${v.input}` },
                { role: "assistant", content: v.output },
              ],
            })
          )

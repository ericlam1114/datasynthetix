      ...
      uploadTask.on('state_changed', 
        ...
        (error) => {
          ...
          if (error.code === 'storage/unauthorized' || error.message?.includes('CORS')) {
            console.error('CORS or authorization error. Check Firebase Storage CORS configuration.');
            reject(new Error('Upload failed due to permission issues. Please try again later.'));
        ...
        () => {
          ...
          getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
            ...
            reject(new Error('File was uploaded but could not get download URL.'));
          });
        }
      );
    });
    ...
    try {
      ...
      if (file) {
        try {
          ...
          console.warn('File upload failed, but continuing with document creation:', error);
          uploadError = error.message || 'File upload failed, but document metadata was saved.';
        }
      }

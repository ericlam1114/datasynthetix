...
export default function DocumentList() {
  ...
  const handleGenerateData = (documentId) => {
    router.push(`/dashboard/process?documentId=${documentId}&autoStart=true`);
  };
  ...
  const handleDeleteDocument = async (documentId) => {
    try {
      ...
      setDeleteStatus({ message: 'Document deleted successfully', type: 'success' });
      ...
      setDeleteStatus({ message: 'Failed to delete document. Please try again.', type: 'error' });
    }
  };
  ...
  return (
    <div>
      {deleteStatus.message && (
        <Alert 
          className={`mb-6 ${
            deleteStatus.type === 'success' ? 'bg-green-50 text-green-800 border-green-200' : 
            deleteStatus.type === 'error' ? 'bg-red-50 text-red-800 border-red-200' :
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {documents.map((doc) => (
            <DocumentCard
              ...
              date={doc.createdAt?.seconds ? new Date(doc.createdAt.seconds * 1000).toISOString() : null}
